...
class FruitsBloc extends Bloc<FruitsEvent, FruitsState> {
  List<FruitModel> favoritesFruits = [];
  ...
  FruitsBloc() : super(FruitsInitial()) {
    on<FruitsEvent>((event, emit) {
      if (event is AddToFavorite) {
        favoritesFruits.add(event.fruit);
        emit(FavoriteFruitsUpdate(fruits: favoritesFruits));
      }
      if (event is RemoveFromFavorite) {
        favoritesFruits.removeWhere((element) => element.id == event.id);
        emit(FavoriteFruitsUpdate(fruits: favoritesFruits));
      }
      if (event is SearchForAllFruits) {
        final newFruits = fruits
            .where((element) => element.name.toLowerCase().contains(event.text.toLowerCase()))
            .toList();
        print(event.text);
        print(fruits.length);
        emit(AllFruitsUpdate(fruits: newFruits));
      }
    });
  }
}
